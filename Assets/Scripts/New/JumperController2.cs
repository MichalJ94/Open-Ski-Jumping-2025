    ...
    (jumperModel.distCollider1.transform.position +
     jumperModel.distCollider2.transform.position) / 2.0f);
        ...
        public void AdjustForceScaleToHillSize()
        {
            ...
            {
                modifier += (130 - hS) / 8;
                startForceScale -= (130 - hS) * (0.00082f * modifier);
            }
        ...
        private void Update()
        {
            ...
            if (state == 2 && !takeoff && Random.value < (float)((0.00002)*(gameplayExtension.turbulenceChance*gameplayExtension.turbulenceChance*0.2f)))
            {

...
namespace OpenSkiJumping.UI.ListView
{
    ...
    public class ListView<TItemData, TItem> : MonoBehaviour where TItem : ListItemBehaviour
    {
        ...
        public Action<TItem> ConfigureItem { get; set; }
        ...
        private bool IsVisible(int index)
        {
            ...
            var firstIndex = Mathf.Max(0, numOutOfView);
            var lastIndex = firstIndex + numVisible - 1;
            return firstIndex <= index && index <= lastIndex;
        }

        public void ScrollToIndex(int index)
        {
            if (items.Count == 0 || IsVisible(index)) return;
        ...
        public void SelectItem(TItemData value)
        {
            SelectedIndex = value == null ? SelectedIndex : Items.IndexOf(value);
            ClampSelectedIndex();
        ...
        private void ShowHelper()
        {
            ...
            content.sizeDelta = direction == ListDirection.Horizontal
                ? new Vector2(prefabSize * items.Count, 0)
                : new Vector2(0, prefabSize * items.Count);
        ...
        public void RefreshShownValue()
        {
            ...
            for (var i = originalIndex; i < numItems; i++)
            {
                ...
                RecycleItem(newIndex, listItems[i]);
                newIndex++;
            }
        ...
        private void RecycleItem(int index, TItem item)
        {
            BindItem(index, item);
            item.Index = index;
        ...
        {
            if (selectionType == SelectionType.None || index == SelectedIndex) return;
